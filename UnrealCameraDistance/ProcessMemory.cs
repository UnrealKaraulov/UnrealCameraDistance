...
public class ProcessMemory
{
    ...
    public bool StartProcess()
    {
        if (this.ProcessName != "")
        {
            ...
            {
                throw new Exception(this.ProcessName + " - не найден ID процесса. Возможно процесс не запущен или не доступен!");
            }
            this.processHandle = this.OpenProcess(this.MyProcess, ProcessAccessFlags.VirtualMemoryOperation | ProcessAccessFlags.VirtualMemoryRead | ProcessAccessFlags.VirtualMemoryWrite );
            if (this.processHandle != IntPtr.Zero)
                return true;

            throw new Exception(this.ProcessName + " - нет прав или антивирус блокирует доступ к процессу.");
    ...
    public void WriteMem(int pOffset, byte[] pBytes)
    {
        ...
        {
            byte[] pbyte = new byte[1] { pBytes[i] };
            ProcessMemory.WriteProcessMemory(this.processHandle, i + pOffset, pbyte, 1, 0);
        }
    }
    ...
    public void WriteMem(int pOffset, byte[] pBytes, bool AddToImageAddress)
    {
        ...
        {
            byte[] pbyte = new byte[1] { pBytes[i] };
            ProcessMemory.WriteProcessMemory(this.processHandle, AddToImageAddress ? this.ImageAddress(i + pOffset) : i + pOffset, pbyte, 1, 0);
        }
